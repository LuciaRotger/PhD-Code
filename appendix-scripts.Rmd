...
```{r, echo=FALSE}
exp.values.Phi = read.table("./C2-EU(Phi2)1000.txt")[,1]
var.values.Phi = read.table("./C2-varU(Phi)1000.txt")[,1]
```
```{r}
exp.values.Phi[3:20]
var.values.Phi[3:20]
...
$\sigma_U^2(\Phi_n)$, for $n=3,\ldots,8$, from the cophenetic indices of all trees in the corresponding $\mathcal{BT}_n$.
...
For $n=3,...,20$ the results have been:
```{r, echo=FALSE}
exp.values.S = read.table("./C2-EU(S2)1000.txt")[,1]
var.values.S = read.table("./C2-varU(S)1000.txt")[,1]
```
```{r}
exp.values.S[3:20]
var.values.S[3:20]
...
To double-check the recurrence, we have computed the values of
$\sigma_U^2(S_n)$, for $n=3,\ldots,8$, from the Sackin indices of all trees in the corresponding $\mathcal{BT}_n$.
...
For $n=3,...,20$ the results have been:
```{r, echo=FALSE}
exp.values.cov = read.table("./C2-EU(SxPhi)1000.txt")[,1]
cov.values = read.table("./C2-covU(SPhi)1000).txt")[,1]
```
```{r}
exp.values.cov[3:20]
cov.values[3:20]
...
```
```{r,echo=FALSE}
ties=read.table("./C2-table-ties.txt")
kable(ties[1:18,])
```
...
```{r C2-ties, out.width = "90%",fig.align="center"}
...
```{r,echo=FALSE}
bin.tb.mean=read.table("./C2-table-tb-means.txt",header = TRUE)
```
...
```
```{r C2-index-TB, out.width = "72%",fig.align="center"}   
...
par(mar=rep(0,4))
```
```{r C4-tree1,fig.width=5,out.width="70%", fig.align="center"}
set.seed(1000)
...
```{r C4-density-tree1,out.width = "70%",fig.align="center"}
...
```{r C4-percentile-tree1,out.width = "70%",fig.align="center"}
...
par(mar=rep(0,4))
```
```{r C4-tree2,fig.width=5,out.width="80%", fig.align="center"}
set.seed(1000)
...
par(mar=c(2,2,2,2))
```
```{r C4-density-tree2,out.width = "70%",fig.align="center"}
distribution(tree.uni,0.5,0.5,db.path = database.location)
```
```{r C4-percentile-tree2,out.width = "70%",fig.align="center"}
...
par(mar=rep(0,4))
```
```{r C4-real-tree,fig.width=8,fig.height=6,out.width="100%",fig.align="center"}  
plot(tree) 
``` 
```{r, echo=FALSE}
par(mar=c(2,2,2,2))
...
```{r C4-heatmap, eval=TRUE, results='hide',warning=FALSE,message=FALSE}
require(ggplot2)
require(reshape2)  
...
write.table(tb.n,"C4-tb-n.txt")
```
The results are available in the files "C4-tb-indices.txt","C4-tb-indices-norm.txt" and "C4-tb-n.txt".
```{r,eval=TRUE,echo=FALSE}
## HIDDEN
tb.idx=read.table("./C4-tb-indices.txt",header=T)
tb.idx.norm=read.table("./C4-tb-indices-norm.txt",header=T) 
tb.n=read.table("./C4-tb-n.txt") 
```
...
## HIDDEN
tb.means=list(read.table("./C4-tb-means-ALL.txt",header=T),
              read.table("./C4-tb-means-norm-ALL.txt",header=T))
tb.vars=list(read.table("./C4-tb-vars-ALL.txt",header=T),
              read.table("./C4-tb-vars-norm-ALL.txt",header=T)) 
```
...
```{r,echo=FALSE}
### HIDDEN
tb.means.reg=read.table("./C4-tb-means-regression.txt",header=TRUE)
tb.vars.reg=read.table("./C4-tb-vars-regression.txt",header=TRUE)
tb.means.reg.norm=read.table("./C4-tb-means-regression-norm.txt",header=TRUE)
tb.vars.reg.norm=read.table("./C4-tb-vars-regression-norm.txt",header=TRUE)
```
...
```{r C4-reg-CL,eval=TRUE,out.width="90%",fig.align="center"}
...
```{r C4-reg-S,eval=TRUE,out.width="90%",fig.align="center"}
...
```{r C4-reg-coph,eval=TRUE,out.width="90%",fig.align="center"}
...
```{r C4-ties,eval=TRUE,out.width="90%",fig.align="center"}
...
```{r C4-spearman,eval=TRUE,out.width="90%",fig.align="center"} 
